package com.netease.protobuf
{
   ...
   public final class TextFormat
   {
      ...
      private static function printValue(output:IDataOutput, fieldDescriptor:BaseFieldDescriptor, value:Object, printSetting:PrintSetting, currentIndent:String = "") : void
      {
         ...
         else
         {
            ...
            else
            {
               ...
               {
                  enumRepeatedFieldDescriptor = fieldDescriptor as RepeatedFieldDescriptor$TYPE_ENUM;
                  if(enumRepeatedFieldDescriptor)
                  {
                     printEnum(output,int(value),enumRepeatedFieldDescriptor.enumType);
                  }
                  else if(fieldDescriptor is FieldDescriptor$TYPE_BYTES || fieldDescriptor is RepeatedFieldDescriptor$TYPE_BYTES)
                  {
                     printBytes(output,ByteArray(value));
                  }
                  else
      ...
      private static function consumeIdentifier(source:ISource) : String
      {
         var b:int = 0;
         skipWhitespace(source);
         for(var nameBuffer:ByteArray = new ByteArray(); true; )
         {
            b = source.read();
      ...
      private static function consumeFieldValue(source:ISource, type:Class) : *
      {
         ...
         switch(type)
         {
            ...
            default:
               ...
               consume(source,123);
               for(message = new type(); true; )
               {
                  if(tryConsume(source,125))
                  {
                     break;
                  }
      ...
      private static function consumeField(source:ISource, message:Message) : void
      {
         ...
         if(repeatedFieldDescriptor)
         {
            ...
            destination.push(Boolean(enumRepeatedFieldDescriptor)?consumeEnumFieldValue(source,enumRepeatedFieldDescriptor.enumType):consumeFieldValue(source,repeatedFieldDescriptor.elementType));
         }
         else
         {
            enumFieldDescriptor = fieldDescriptor as FieldDescriptor$TYPE_ENUM;
            message[fieldDescriptor.name] = Boolean(enumFieldDescriptor)?consumeEnumFieldValue(source,enumFieldDescriptor.enumType):consumeFieldValue(source,fieldDescriptor.type);
         }
      }
